...
class ActivityLogMiddleware:
    ...
    def log_activity(self, request, response, duration):
        ...
        if any(path in request.path for path in ['/static/', '/media/', '/favicon.ico']):
            return
        ...
        if 200 <= status_code < 300:
            ...
            logger.warning(f"[CLIENT ERROR] {user_info} - {method} {path} - {status_code}")
        else:
            logger.error(f"[SERVER ERROR] {user_info} - {method} {path} - {status_code}")
